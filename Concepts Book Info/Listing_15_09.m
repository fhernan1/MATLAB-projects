% Listing 15-9 - Raw rocket values
clear 
clc
 ...
 lv = length(v); dt = 0.2; t = (0:lv-1) * dt;
 h = dt * cumsum(v);
 acc = diff(v) ./ diff(t);
 plot(t, v, t, h/5, t(2:end), acc)
 legend({'velocity' 'altitude/5' 'acceleration'})
 title('vel, alt and acc measurements of a rocket')
 xlabel('time (sec)')
 ylabel('v (m/s), h(m/5) and acc(m/sec^2)')
